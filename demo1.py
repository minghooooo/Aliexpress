...
def parseGoodsHTML(alist,neededDepth):
    for a in alist:
        ...
        while depth <= neededDepth:
            if depth == 1:
                GoodsNames =  re.findall(r'\"title\":\".+\"',a)
                GoodsUrls = re.findall(r'\"productDetailUrl\":\".+\"',a)
                ...
                GoodsUrls = re.findall(r'\"productDetailUrl\":\".+\"',nextpageHTML)
                soup2 = BeautifulSoup(nextpageHTML,'html.parser')
                linkToNext = soup2.find('link',attrs={'rel':'next'})
                NextUrl = linkToNext['href']
            for i in range(len(GoodsNames)):
                hrefDic = {}
                hrefDic['name'] = eval(GoodsNames[i].split(':')[1])
                hrefDic['url'] = eval(GoodsUrls[i].split(':')[1])
                parseGoods(hrefDic['url'],GLs) #进入"productDetailUrl" 
            nextpageHTML = NextPage(NextUrl) # 判断是否爬取完当页商品，爬完则在aHTML中获取下一页url
            depth = depth+1
        # 爬完这一页的商品后，保存成csv，用类名
        GoodsHeader={'price','name','like','comments','tradecount','star'}
        with open(a.string +'.csv','w',newline='') as fp:
